...
class SafeLine {
    ...
    func didHitPlayer() -> Bool {
        ...
        let r: CGFloat = (self.width / 2) - Player.radius
        ...
        if circleDidHitLine(playerPoint: playerPoint, r: r) == true {
            ...
            let playerPoint2 = CGPoint(x: player.x, y: player.y)
            let r2: CGFloat = (self.width / 2) + Player.radius
            
            if circleDidHitLine(playerPoint: playerPoint2, r: r2) == true {
                
                player.isPartSafe = true
                
                let newX = ((self.endX - self.startX) * constant) + self.startX
                let newY = ((self.endY - self.startY) * constant) + self.startY
                
                player.tempRadius = (Player.radius - ((self.width / 2) - distance(x1: newX, y1: newY, x2: player.x, y2: player.y))) / 2
                ...
                if player.x - newX == 0 {
                    ...
                    slope = (player.y - newY) / (player.x - newX)
                }
                ...
                if player.x < newX {
                    ...
                    player.tempX = newX - tempX
                    player.tempY = newY - (tempX * slope)

                } else {

                    player.tempX = tempX + newX
                    player.tempY = (tempX * slope) + newY
                }
    ...
    func didHitGoal() -> Bool {
        ...
        let goalPoint = CGPoint(x: goal.x, y: goal.y)
        let r: CGFloat = (self.width / 2) - (Goal.width / 2)
        
        if circleDidHitLine(playerPoint: goalPoint, r: r) == true {
            return true
        }
